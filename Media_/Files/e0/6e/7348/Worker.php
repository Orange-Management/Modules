...
if(array_key_exists($this->request->getAccount(), $restricted) && in_array($this->request->getData('source'), $restricted[$this->request->getAccount()])) {
    $source = $this->request->getData('source');
} elseif(array_key_exists($this->request->getAccount(), $restricted)) {
    $source = $restricted[$this->request->getAccount()][0];
...
$file = fopen(ROOT_PATH . '/' . $rcoll['NOGIT.csv']->getPath(), 'r');
...
$file = fopen(ROOT_PATH . '/' . $rcoll['AreaManagerClientsNOGIT.csv']->getPath(), 'r');
...
$file = fopen(ROOT_PATH . '/' . $rcoll['AreaManagerUG3YClientsNOGIT.csv']->getPath(), 'r');
